...
06/03/23 Jemima Pilgrim-Morris: unprepped IR-bSSFP
"""
...
# JPM: no analytical solution for unprepped IR-bSSFP
...
if __name__ == "__main__":
        ...
        # JPM: to replicate the ISMRM abstract, do not perform normalisation
   ...
   #             finite_data[i]  = finite_data[i] * nom_fac[i]
        ...
        fig = plt.figure(figsize=(15, 20), dpi=120, edgecolor='w')
        ...
        for f in range(0,len(finite_data)):
                ...
                c=0
                ax1.plot(finite_data[f], '-', color=COLOR[2], alpha=1, linewidth=LW, label="$\\bf{DQ}$(h=1%)")
                ...
                # JPM: add difference plot, scale by maximum error

                abs_diff = np.abs(finite_data[f] - sa_data[f])

                ax2=ax1.twinx()
                ax2.tick_params(axis='both', which='major', labelsize=FS)
                ax2.yaxis.set_major_formatter(FormatStrFormatter('%.3f'))
                ax2.yaxis.set_major_locator(plt.MaxNLocator(5))
                ax2.plot(abs_diff/max(abs_diff), '-', alpha=0.5, linewidth=LW, label = "$\\bf{|DQ - SAB|}$")
                ax2.set_ylabel("$\\bf{|DQ - SAB|}$", fontsize=FS)
                ...
                axin = ax1.inset_axes((inset_loc[f][0], inset_loc[f][1],.4,.2))

                

                axin.plot(finite_data[f], '-', color=COLOR[2], alpha=1, linewidth=LW, label="$\\bf{DQ}$(h=1%)")
                ...
                if (0 < np.max(np.array(finite_data[f][x1:x2]))):
                        y1 = np.min(np.array(finite_data[f][x1:x2]))*0.99
                        y2 = np.max(np.array(finite_data[f][x1:x2]))*1.01
                else:
                        y2 = np.min(np.array(finite_data[f][x1:x2]))*0.99
                        y1 = np.max(np.array(finite_data[f][x1:x2]))*1.01
        ...
        axLine = axLine1 + axLine2
        axLabel = axLabel1 + axLabel2

        fig.legend(axLine, axLabel, loc = 'lower center', bbox_to_anchor=(0.5, 0), fontsize=FS-5, fancybox=True, shadow=True, ncol=4)
        #fig.legend(loc = 'lower center', bbox_to_anchor=(0.5, -0.05), fontsize=FS-5, fancybox=True, shadow=True, ncol=4)
